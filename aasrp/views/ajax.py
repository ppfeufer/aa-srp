...
def dashboard_srp_links_data(
    ...
    for srp_link in srp_links:
        ...
        data.append(
            {
                "srp_name": srp_link.srp_name,  # Name of the SRP link
                "creator": get_main_character_name_from_user(
                    user=srp_link.creator
                ),  # Creator's main character name
                "fleet_time": srp_link.fleet_time,  # Time of the fleet
                "fleet_type": fleet_type,  # Type of the fleet
                "fleet_doctrine": srp_link.fleet_doctrine,  # Doctrine of the fleet
                "aar_link": aar_link,  # AAR link (if available)
                "srp_code": {
                    "display": srp_code_html,
                    "sort": srp_link.srp_code,
                },  # SRP code with optional icon
                "srp_costs": srp_link.total_cost,  # Total SRP costs
                "srp_costs_html": {
                    "display": localized_isk_value(
                        srp_link.total_cost
                    ),  # Localized ISK value for display
                    "sort": srp_link.total_cost,  # Value for sorting
                },
                "srp_status": srp_link.srp_status,  # Status of the SRP link
                "pending_requests": srp_link.pending_requests,  # Number of pending requests
                "actions": dashboard_action_icons(  # Available actions
    ...
    for srp_request in requests:
        ...
        if srp_request.killboard_link:
            try:
                ...
                # If the ship is missing from the EveType table, attempt to re-add it
        ...
        data.append(
            {
                ...
                "zkb_link": killboard_link,  # Killboard link
                "zkb_loss_amount_html": {
                    "display": localized_isk_value(
                        srp_request.loss_amount
                    ),  # Localized ISK loss amount
                    "sort": srp_request.loss_amount,  # Loss amount for sorting
                },
        ...
        data.append(
            {
                ...
                },  # Ship details with killboard link
                "ship": srp_request.ship.name,  # Name of the ship
                "zkb_link": killboard_link,  # Killboard link
                "zkb_loss_amount_html": {
                    "display": localized_isk_value(
                ...
                "zbk_loss_amount": srp_request.loss_amount,  # Loss amount
                "payout_amount_html": {
...
def srp_request_additional_information(
    ...
    try:
        ...
        # Retry generating the ship render icon HTML
        ship_render_icon_html = get_type_render_url_from_type_id(
            evetype_id=srp_request.ship_id,
            evetype_name=srp_request.ship.name,
            size=64,
